...
class GRUObservationCellLogvar(torch.nn.Module):
    ...
    def forward(self, h, p, X_obs, M_obs, i_obs):
        ...
        temp = h.clone()
        #edited
        temp[i_obs] = self.gru_d(gru_input, h[i_obs])*M_obs[i_obs]+temp[i_obs]*(1-M_obs[i_obs])
        h = temp
